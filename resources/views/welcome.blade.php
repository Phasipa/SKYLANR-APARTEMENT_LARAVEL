...
<body>
                    ...
                    @foreach ($catalog as $data)
                        <div class="card">
                            <div class="card-image">
                                <img src="{{ asset('img_catalog/' . $data->gambar) }}" alt="tidak ada gambar" />
                            </div>
                            <div class="card-content">
                                ...
                                <button class="btn_belanja" type="button"
                                    onclick='bukaModal({{ $data->id_catalog }})'>Beli</button>
                            </div>
                        </div>
                        <!-- Modal 1 -->
                        <div id="myModal{{ $data->id_catalog }}" class="modal-container"
                            onclick="tutupModal({{ $data->id_catalog }})">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary"
                                            onclick="tutupModal({{ $data->id_catalog}})">Keluar</button>
                                        <button type="button" class="btn btn-yellow"
                                            onclick="bukaModal2({{ $data->id_catalog}})">Lanjutkan</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Modal 2 -->
                        <div id="myModal2{{ $data->id_catalog}}" class="modal-container"
                            onclick="tutupModal2({{ $data->id_catalog}})">
                            <div class="modal-dialog" onclick="event.stopPropagation()">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <form action="{{ route('transaction') }}" method="post">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary"
                                                onclick="kembaliKeModalPertama({{ $data->id_catalog}})">Kembali</button>
    ...
    <script>
        function bukaModal(catalogId) {
            var modal = document.getElementById('myModal' + catalogId);
            modal.style.display = 'flex';
        }

        function tutupModal(catalogId) {
            var modal = document.getElementById('myModal' + catalogId);
            modal.style.display = 'none';
        }

        function bukaModal2(catalogId) {
            tutupModal(catalogId); // Tutup modal pertama
            var modal2 = document.getElementById('myModal2' + catalogId);
            modal2.style.display = 'flex';

            // Ambil nilai dari input fields pada modal pertama
            var modal1 = document.getElementById('myModal' + catalogId);
        ...
        function tutupModal2(catalogId) {
            var modal2 = document.getElementById('myModal2' + catalogId);
            modal2.style.display = 'none';
        }

        function kembaliKeModalPertama(catalogId) {
            tutupModal2(catalogId);
            bukaModal(catalogId);
        }
